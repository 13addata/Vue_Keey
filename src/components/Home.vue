      ...
      <transition name="logintrans">
        <div class="loginbox" v-if="showLogin">
          ...
          <div class="login-input">
            ...
            <input type="password" placeholder="Password" v-model="password" @keyup.enter="loginAcct">
          </div>
          <a href="javascript:void(0);" class="btn-login" @click="loginAcct"><i class="fa fa-check"></i></a>
        </div>
      </transition>
    </div>
      ...
      this.$http.post('/auth/user', obj) // post data to backend(Koa)
        .then((res) => { // data return from axios is in res.data
          if(res.data.success){ 
            ...
            this.$message.error(res.data.info); // ======Login fail 
            sessionStorage.setItem('user-token',null); // delete token
          }
        }, (err) => {
